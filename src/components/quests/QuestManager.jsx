    ...
    <div className="flex gap-8 items-start relative pb-24 h-full">
      <div className="flex-1 w-full min-w-0 p-8 bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col h-fit">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Define broad story quests. Quests are organizational tags for scenes.</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {Object.values(quests).map(quest => (
          <div key={quest.id} id={quest.id} className="scroll-mt-8 bg-white border border-gray-200 rounded-2xl p-4 flex items-center justify-between shadow-[0_2px_10px_rgba(0,0,0,0.02)] group hover:border-indigo-200 transition-colors">
            <div className="flex items-center gap-4">
              <span className="font-mono text-sm font-bold text-indigo-400 bg-indigo-50 px-3 py-1.5 rounded-lg">{quest.id}</span>
              <DebouncedInput
                type="text"
                value={quest.name}
        ...
        {Object.keys(quests).length === 0 && (
          <div className="text-center py-12 bg-gray-50 border border-gray-200 border-dashed rounded-2xl flex flex-col items-center">
             ...
             <p className="text-gray-500 font-medium tracking-wide">No quests exist yet.</p>
          </div>
        )}
      </div>
      </div>
