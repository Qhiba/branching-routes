...
function WarpEntranceNode({ id, data }) {
  ...
  const className = `story-node warp-entrance-node ${nodeState ? 'story-node--' + nodeState : ''} ${isSeen || (isEditorSeen && !isCampaignActive) ? 'story-node--seen' : ''} ${isCoverageGap ? 'story-node--coverage-gap' : ''}`.trim();

  return (
    <div className={className}>
      <Handle type="target" position={Position.Left} />

      <div className="story-node__type-bar warp-entrance-node__type-bar">
        <span className="story-node__type-label">WARP ENTRANCE</span>
        {isOrphaned && (
...
export default memo(WarpEntranceNode);
